...
for recording_path in recording_paths:
    ...
    if "OF1" in recording_path:
        stagein_dict[f"{active_projects_path / recording_path}"] = (
            data_folder / "OF" / recording_folder_name
        )
    else:
        stagein_dict[f"{active_projects_path / recording_path}"] = (
            data_folder / "VR" / recording_folder_name
        )

stageout_dict = {}
for session in sessions:
    stageout_dict[deriv_folder / f"M{mouse:02d}/D{day:02d}/{session}/{protocol}"] = (
        eddie_active_projects / "Chris/components_real_data/Wolf_M5_D4/"
    )

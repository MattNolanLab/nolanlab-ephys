...
def test_three_preprocessing_steps():
    ...
    pp_dict_of_recs_from_functions = whiten(
        bandpass_filter(common_reference(dict_of_recs)), seed=1205
    )
...
def test_loading_from_analyzer(create_cache_folder):
    ...
    _ = create_sorting_analyzer(
        sorting=sorting, recording=pp_recording, format="zarr", folder=analyzer_zarr_folder
    )

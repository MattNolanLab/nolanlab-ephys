"""
Note: This file is temporarily added to this repo branch for a presentation.

This code was originally developed alongside `nolanlab-ephys`, then merged into SpikeInterface.
This is the final SpikeInterface implementation, which was merged into si here:
...
pp_names_to_functions = {
    preprocessor.__name__: preprocessor for preprocessor in preprocessor_dict.values()
}
pp_names_to_classes = {
    pp_function.__name__: pp_class for pp_class, pp_function in _all_preprocesser_dict.items()
}
...
\n>>> print(pp_names_to_functions.keys())"
    ...
    def _apply(self, recording, apply_precomputed_kwargs=False):
        ...
        for preprocessor_name, kwargs in self.preprocessor_dict.items():
            ...
            non_rec_kwargs = {
                key: value for key, value in kwargs.items() if key not in dont_apply_kwargs
            }
...
def get_preprocessing_dict_from_file(recording_dictionary_path):
    ...
    elif str(recording_dictionary_path).endswith(".pkl") or str(recording_dictionary_path).endswith(
        ".pickle"
    ):
        import pickle
...
def _load_pp_from_dict(prov_dict, kwargs_dict):
    ...
    prov_dict_to_kwargs_dict = lambda x: (
        _load_pp_from_dict(x, kwargs_dict) if is_dict_extractor(x) else x
    )
